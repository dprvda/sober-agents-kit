...
| `check_doc_freshness` | docs drifting from the code they track (`tracks_dir:`) or undated snapshots (`frozen_at:`). Enforced ONLY at the session-wrap commit (the one staging a handoff) — mid-session commits iterate unburdened. Audit any time: `--json`; enforce always: `AGENT_KIT_DOC_FRESHNESS_ALWAYS=1` |
